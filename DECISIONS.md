...
| 11.8 | The `epubjs` advisory | **Filed as #3**, not fixed | Applying the override now | It was filed on the belief that the only fix was a semver-major `epubjs` bump. **That was wrong**: `npm audit`'s suggested `0.4.2` is a March 2018 release, older than the 0.3.93 in use, and it depends on the ancient `xmldom@0.1.x`. The real fix is a one-line `overrides` entry, which was later tried and passed everything (see F3). Also found: xmldom is never invoked in a browser | **you** | session |
...
| F3 | **The `epubjs` advisory** (issue #3) | Nothing — it is ready. It was tried and reverted, awaiting your go | Add `"overrides": {"@xmldom/xmldom": "0.8.15"}` (verified: audit 0, suite + build + EPUB session unchanged); accept and document it as unreachable; wait for an epubjs release. **Not** `epubjs@0.4.2`: that is a 2018 downgrade | The override. It costs one line and about ten minutes, and needs a redeploy because the bundled code changes. Severity is low regardless: epub.js uses the native `DOMParser` in a browser and only falls back to xmldom when `DOMParser` is undefined, `forceXMLDom` is set (nothing sets it), or in IE |
