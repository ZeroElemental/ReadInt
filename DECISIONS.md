...
| 11.8 | The `epubjs` advisory | **Filed as #3**, not fixed | Bumping to 0.4.2 now; pinning an override | The only offered fix is a semver-major bump that would need all of Phase 6 re-verified | **you** | session |
...
| F3 | **The `epubjs` advisory** (issue #3) | Whether the threat model (a self-chosen file, nothing server-side) makes it urgent | Bump to 0.4.2 and re-verify Phase 6; pin an override if one exists in range; accept and document | Bump, on its own branch, with the Phase 6 verification re-run |
