...
| F8 | **`functions/` dependency audit** (issue #5) | Whether 11 moderate transitive findings (all via `firebase-functions`) are worth a bump | `npm audit fix` for the two with a non-breaking fix (`express`/`qs`, `gaxios`/`uuid`); bump `firebase-functions` to 7.x (a semver-major, needs the function re-verified); accept | The non-breaking fixes first — they cover the `qs`/`express` pair, the only plausibly reachable one, since the function's code imports only `firebase-functions` and never calls `uuid` directly; treat the major bump as its own task |
